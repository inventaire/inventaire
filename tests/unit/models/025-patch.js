...
describe('patch', function() {
  describe('create', function() {
    it('should throw if passed an invalid user id', function(done){
      const params =
    ...
    it('should throw if passed identical objects', function(done){
      ((() => Patch.create({ userId, currentDoc, updatedDoc: currentDoc })))
      .should.throw();
      return done();
    });

    it('should throw if there are no changes', function(done){
    ...
    it('should throw if passed an updated doc without id', function(done){
    ...
    it('should throw if passed an invalid doc object', function(done){
    ...
    it('should return an object of type patch', function(done){
    ...
    it('should return with user set to the user passed', function(done){
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
      patch.user.should.equal(userId);
      return done();
    });

    it('should return with a timestamp', function(done){
      const now = _.now();
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
    ...
    it('should return with a patch object', function(done){
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
      patch.patch.should.be.an.Array();
      patch.patch.forEach(function(op){
    ...
    it('should ignore data out of versionned attributes', function(done){
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
      const updateFromPatch = jiff.patch(patch.patch, currentDoc);
    ...
    it('should return with an _id built from the document id and the version', function(done){
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
    ...
    return it('should accept an arbitrary context object', function(done){
  ...
  return describe('revert', function() {
    it('should revert a patch', function(done){
      const patch = Patch.create({ userId, currentDoc, updatedDoc });
    ...
    it('should revert an update patch', function(done){
    ...
    it('should revert a patch between patches', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
    ...
    it('should revert a patch between more patches', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
    ...
    it('should revert a delete patch', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
    ...
    it('should revert a delete patch after the doc was re-edited', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
    ...
    it('should revert a patch between patches on different claims', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
    ...
    return it('should revert successive patches', function(done){
      const authorDocUpdatedA = _.cloneDeep(authorDoc);
      authorDocUpdatedA.claims['wdt:P50'] = [ 'wd:Q535' ];
