...
export async function mergeOrCreateOrUpdateTask (entitiesType, fromUri, toUri, fromEntity, toEntity, userId) {
  const labelsMatchMergePromise = labelsMatchMergeByType[entitiesType]
  if (labelsMatchMergePromise) {
    const isMerged = await labelsMatchMergePromise({ fromUri, toUri, fromEntity, toEntity, userId })
