...
import type { ActionController, HttpMethod } from '#types/controllers'
...
for (const [ method, methodParams ] of objectEntries(localEntitiesControllersParams)) {
  federatedEntitiesControllersParams[method] = {}
  for (const [ accessLevel, actionControllers ] of objectEntries(methodParams)) {
    ...
    federatedEntitiesControllersParams[method][localAccessLevel] ??= {}
    for (const [ actionName, actionController ] of objectEntries(actionControllers)) {
      const controller = proxiedController(accessLevel, method, actionName as string, actionController)
      federatedEntitiesControllersParams[method][localAccessLevel][actionName] = controller
    }
  }
}

function proxiedController (accessLevel: AccessLevel, method: HttpMethod, action: string, actionController: ActionController) {
  if (!(accessLevel === 'public' || accessLevel === 'authentified')) {
    return () => { throw newError('This endpoint is closed in federated mode', 400, { endpoint: `${method.toUpperCase()} /api/entities?action=${action}` }) }
  }
...
export const federatedEntitiesControllers = methodAndActionsControllersFactory(federatedEntitiesControllersParams)
