...
async function updateInvClaims (entity: InvEntityDoc, seedClaims: Claims, imageUrl: Url | undefined, reqUserId: UserId, batchId: BatchId) {
...
async function updateWdClaims (entity: SerializedWdEntity, seedClaims: Claims, imageUrl: Url | undefined, reqUserId: UserId) {
  ...
  const imageHash = await getImageHash(entity, imageUrl)
  await updateWdEntityLocalClaims(user, id, 'invp:P2', null, imageHash)
}
...
async function getImageHash (entity: InvEntity | SerializedWdEntity, imageUrl: Url | undefined) {
  if (!imageUrl) return
