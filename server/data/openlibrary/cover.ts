...
export default async function (openLibraryId: string, entityType: string) {
...
async function checkCoverExistance (url: AbsoluteUrl) {
  ...
  const { statusCode } = await requests_.head(`${url}?default=false` as AbsoluteUrl)
