...
describe('followers', () => {
  describe('users', () => {
    ...
    it('should paginate followers URLs', async () => {
      ...
      const firstfollowersPage: Url = `${endpoint}${username}&offset=0&limit=1`
      ...
      const nextfollowersPage: Url = `${endpoint}${username}&offset=1`
      const res2 = await publicReq('get', nextfollowersPage)
      res2.orderedItems.length.should.equal(2)
  ...
  describe('with-remote-actors', () => {
    const infoEndpoint = '/api/activitypub?action=followers&with-remote-actors=true&name='
